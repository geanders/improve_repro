...
useful to start by defining the problem \citep{osann2020design}. What are you aiming
to achieve with your file organization system?

Here are some key goals to consider for a research project directory template,
based on our own experiences and the advice of others \citep{marwick2018packaging, bertin2021creating}:
...
\item
  Uses simple file formats and tidy structures for recording data whenever
  possible (a larger number of simple files is typically easier to organize and work
  with than a smaller number of complex files)
\item
  Keeps all files for a research project within a single directory
\item
  Facilites creation of reports and analysis that incorporate data from different assays
  for an experiment
\item
  Treats data files as read-only
\item
  Uses meaningful names for files and subdirectories
\item
  Avoids or removes unneccessary files
\item
  Uses subdirectories to organize files into a hierarchical structure
\item
  Makes it easy to share all project files across the team
\item
  Makes it easy to share project files on a public repository
\item
  Makes it easy to track changes to files for a project
\item
  Allows easy navigation and discoverability (module 2.6) by a new user
\item
...
{[}Anecdotes / examples for prototyping{]}
...
what the typical size, structure, and contents of each type of file will be. For
...
for data from equipment, these can just be one or more example files from the
...
{[}Reports{]} With the previous steps, you will have determined the types of files you
...
easy to work within, as you continue work on your project.

Adam Savage, the host of the \emph{Mythbusters} television show who we introduced in
...
organization is one that not only helps you find what you need but also encourages
your creativity in solving problems. He notes:
...
We'll go through each of these to discuss what might be included in each
of these areas, as well as how it might make sense to name subdirectories
...
read in to a separate program for analysis. That way, you can work with the
...
There are different recommendations on how to name subdirectories with data.
Several papers recommend having separate subdirectories for the raw data versus
intermediate data. Some researchers have suggested naming the subdirectory for
raw data as ``data-raw'' and the one for intermediate data as ``data''
\citep{vuorre2021sharing, prodigenr}. Others have suggested naming the raw data
subdirectory as ``data'' and the one for intermediate data ``outputs''
\citep{blischak2019creating}. Either or these choices---or a reasonable
alternative---is fine, as long as you use your naming scheme consistently across
projects. In some cases, the raw data directory may also keep the code scripts
that you used to create the intermediate data from those raw data \citep{prodigenr}.
...
cases, in might include code for functions that you plan to reuse within
...
files, you could keep these files (the ones you should work on by hand) in one
subdirectory and have another subdirectory to store the output of those
RMarkdown files (the generated reports in a format like PDF or Word, which you
should treat as read-only if they were generated from an RMarkdown file)
\citep{blischak2019creating}. These two subdirectories could be named ``analysis'' and
``output'', respectively \citep{blischak2019creating}. Another article recommends
using separate subdirectories for different types of report outputs, for
example ``posters'', ``manuscript'', and ``slides'' \citep{vuorre2021sharing}.
...
\textbf{Meta-documentation subdirectories or files}

The final major area to cover in your project directory are files for metadata
and ``meta-documentation''. This is information that describes your project as
a whole. In some cases you might store this information in subdirectories,
but in many cases, this information might alternatively go in a single file
at the main level of the project directory.

There are a number of pieces of information that you may want to include
in this metadocumentation. It could include, for example, information about
the experiment that the project covers, things like which type of model animal
you were using or which treatment you were testing.

It can also include information
related to the code analysis. One piece of information that's very important,
for example, is a list of the dependencies and versions of software. For
example, if you used R for analysis, which version of R did you use, and which
packages did you use to supplement the base R distribution? For the packages,
which version of each did you use?

The metadocumentation can also provide some information on who was involved
...
If you are planning to use GitHub as a way to share the project directory
(see modules 3.10 and 3.11), you will find it useful to create the README file
using a file format called ``Markdown''. {[}more about this{]} {[}Automatically
renders in a nice format when you put it on GitHub{]}
...
{[}Maybe move this before designing the conceptual structure{]}

Once you have a blueprint for a template for a project directory, you can create
this as a ``physical template'' directory on your computer. This will serve as a
prototype to test.
...
reports (modules 3.7--3.9), you can include those within this structure. When
...
work with the data you collect for the project.
...
project template). First, you will find the project directory template in your
...
In this module, as well as the following two, we'll be exploring how you can use
...
improve the reproducibility of the studies.

\subsection{Step 1: Survey of data collected for the projects}\label{step-1-survey-of-data-collected-for-the-projects}

The first step in developing a project template is to take a survey of the
...
some metadata recorded for each study. Figure \ref{fig:metadata} gives an
...
the tested treatment, the batch number of that drug.
...
As you can see, these data were all recorded in a format designed for the tidy
...
can then be done in separate files.
...
The ``tidyverse'' approach is an approach to using R that has grown enormously in
popularity in recent years. Most R courses and workshops for beginning
programmers are now structured around this approach. It provides a powerful yet
flexible approach for working with data in R, and it one of the easier ways to
start learning R. In a previous module (module 2.3) we described the tidyverse
approach in conjunction with talking about the power of the tidy data format. In
this module, we'll go deeper into specific tools under this approach that can be
used for common data preprocessing tasks when working with biomedical data, as
well as provide information on more resources that can be used to continue
...
tends to do one thing simply but well. For example, one function (\texttt{select}) just
limits the data to a subset of its original columns; another (\texttt{mutate}) adds or
changes values in columns of the dataset, while another (\texttt{distinct}) limits the
dataframe to remove any rows that are duplicates.

This makes it fairly easy to start learning the tidyverse approach, as each of
the functions that you learn as you begin does one thing that is fairly
straightforward. These small, simple steps can be combined together in different
patterns to add up to complex operations on the data, while keeping each step
very simple and clear. Since the data stays in a standard and simple object
type, it is easy to check in on your data at any stage, as the common
visualization tools for this approach (from the \texttt{ggplot2} package and its
extensions) can be always be applied to data stored in a tidy dataframe.
As you get better and better as coding using this approach, you often find that
it is not because you are using more complex functions, but rather that you're
becoming more clever at combining sets of simple functions in interesting ways.
...
deviation. This type of scaling is often required before using some of the
techniques for dimension reduction (e.g., principal components analysis) or
clustering, to ensure that the unit of measurement of each column does not
influence its weight in later analysis. For example, if you were clustering
observations using measurements for each subject that included their weight, you
don't want to get different results depending on whether their weight is
measured in grams versus pounds, and this type of scaling can help avoid any of
those differences based on the units used for measurements.
