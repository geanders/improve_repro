...
\hypertarget{improving-scientific-research-with-version-control}{%
\subsection{Improving scientific research with version control}\label{improving-scientific-research-with-version-control}}
...
of the data recorded in the lab \citep{mascarelli2014research}.
...
notebooks \citep{butler2005electronic}.
...
\citep{giles2012digital}. These were adopted more early in pharmaceutical industrial
...
laboratories \citep{giles2012digital, butler2005electronic}.
...
Google applications, OneNote, and Evernote \citep{perkel2011coding, kwok2018lab, giles2012digital, powell2012lab}.
...
\citep{giles2012digital, butler2005electronic}.
...
time-consuming to pursue using paper-based methods.'' \citep{giles2012digital}
\end{quote}

Large funding agencies often now require a plan for managing and sharing data. \ldots{}
...
writing scientific manuscripts \citep{perez2016ten} and grant proposals {[}examples{]}.

The combination of git and GitHub can also help as a way to backup study data
and code \citep{blischak2016quick, perez2016ten, perkel2018git}. Together, git and
...
but with the added capabilities and control of Git.'' \citep{blischak2016quick}
\end{quote}
...
present on each user's storage device.'' \citep{perez2016ten}
\end{quote}

\hypertarget{tracking-experimental-data-with-version-control-tools}{%
\subsection{Tracking experimental data with version control tools}\label{tracking-experimental-data-with-version-control-tools}}
...
control something, does not mean that you \emph{should}. Git works best for plain,
...
versions (using \texttt{git\ diff}). You can commit a non-text file, but a full copy
of the file will be saved in each commit that modifies it. \ldots{} Things \emph{not} to
version control are large data files that never change, binary files (including
Word and Excel documents), and the output of your code.'' \citep{blischak2016quick}
\end{quote}
...
and compare file content across revisions.'' \citep{perez2016ten}
\end{quote}
...
to download it to her machine as needed, e.g., using \texttt{wget}, \texttt{curl}, or \texttt{rsync}.''
\citep{blischak2016quick}
\end{quote}

\begin{quote}
If you need to use git for really large files that \emph{do} change, you may want to
...
downloads the old version of the large file from the remote server.'' \citep{blischak2016quick}
\end{quote}

The ability in git to compare differences between two plain text files derives from
a longstanding Unix command line tool called \texttt{diff}.
{[}diff Unix tool{]}
...
invented the \emph{patch}(1) utility. The \emph{patch} program, a simple tool that applies
changebars generated by \emph{diff}(1) to a base file, meant that Unix developers could
...
and re-converge. The \emph{patch} program did more than any other single tool to
enable collaborative devvelopment over the Internet---a method that would revitalize
Unix after 1990.'' \citep{raymond2003art}
\end{quote}
...
powerful algorithm. Perhaps the clearest example of this is \emph{diff}(1), the Unix
tool for reporting differences between related files. This tool and its dual,
\emph{patch}(1), have become central to the network-distributed development style of
...
without necessarily understanding the central algorithm perfectly.'' \citep{raymond2003art}
\end{quote}
...
diff instead; specifically, send the output of the \emph{diff}(1) command run to compare the
baseline distributed version against your modified version.'' \citep{raymond2003art}
\end{quote}

\begin{quote}
``The \emph{diff}(1) command and its dual, \emph{patch}(1), are the most basic tools of open-source
...
respect for his time.'' \citep{raymond2003art}
\end{quote}
...
\citep{blischak2016quick}. Sharing code and data allows a more complete assessment of
...
\citep{perez2016ten}. In GitHub, a repository's setting can be changed from ``Private''
...
data analysis are made publicly available.'' \citep{perez2016ten}
\end{quote}

\begin{quote}
``{[}For many journals{]} Sharing data, code, and other materials is quickly moving
from `desired' to `required'. \ldots{} Granting agencies, philanthropic foundations,
...
requirements in the last decade.'' \citep{blischak2016quick}
\end{quote}
...
\citep{perez2016ten}. In Chapter {[}x{]} we provide guidance on getting started with
...
to want to skip if they have to be done by hand.'' \citep{raymond2003art}
\end{quote}
