...
tools more focused on other steps, like data analysis {[}Tidy data{]}. The word
``tidy'' is not meant to apply that other formats are ``dirty'', or that they
include data that is incorrect or subpar. In fact, the same set of datapoints
could be saved in a file in a way that is either ``tidy'' (in the sense of {[}Tidy
data{]}) or untidy, depending only on how the data are organized across columns
and rows.
...
things like calendar plots ({[}package{]}), gene arrow maps (\texttt{gggene}), network
