...
\newthought{Science advances by building on} previous results, an idea that
...
To help you understand the tidy data format the Wickham developed, let's start
...
\item
  The data collection file or spreadsheet does \textbf{not} include extra elements
...
certain drug over time. In this case, the \emph{unit of analysis} might be the drug,
...
infection status), then the \emph{unit of measurement}---the level at which each data
...
such as organizing files and directories and documenting progress \ldots{} are
important because poor organizational choices can lead to significantly slower
research progress.'' \citep{noble2009quick} In fact, if files are organized in a
...
consistent structure \citep{buffalo2015bioinformatics}.
...
missing information.'' \citep{baker2016quality} An article on organizing computational
...
go unanswered''. \citep{marwick2018packaging}
\end{quote}
...
in a single directory within the file system of a computer \citep{noble2009quick}.
...
everything.'' \citep{buffalo2015bioinformatics}
\end{quote}
...
online version control platforms \citep{vuorre2021sharing}. (This also helps you to
track changes to files across the project \citep{vuorre2021sharing}). In later
modules in this book, we will introduce \texttt{git} version control software and the
...
``discoverability''. In the classic design book \emph{The Design of Everyday
...
out exactly where and what everything is.'' \citep{buffalo2015bioinformatics}
...
organizing project files \citep{marwick2018packaging}. As an example of how standards
...
numerous data and code files).'' \citep{marwick2018packaging}
\end{quote}
...
To ``define the problem'' \citep{osann2020design}:
...
\item
  ...
  so it needs to be flexible enough to accommodate that
\item
...
reproduced as well.'' \citep{marwick2018packaging}
\end{quote}
...
This is a process of \emph{designing}, and so you will find it helpful to follow
principles that facilitate the design process. For example, as you design, it's
useful to keep in mind the resources that you have available \citep{osann2020design}.
...
improve based on what you find out in practice \citep{osann2020design}. In this case,
...
the design process is to observe \citep{osann2020design}. This lets you get a
...
\citep{osann2020design}. This allows you to think about a variety of needs and
...
within a subdirectory called ``data\_raw'' and the processed (gated) csv files
...
As you do decide how to organize a project directory into subdirectories, it
...
typically collect for this type of project.
...
In general, as you design the structure of subdirectories, as well as pick
...
he designs the layout and organization of his shops:
...
As you design, keep in mind that you want to organize and to do it consistently:

\begin{itemize}
\tightlist
\item
  Put all code, data, and output for a project in a single directory
\item
  Organize those directories into subdirectories
\item
  Name those subdirectories consistently
\item
...
Here is the suggestion from another paper \citep{marwick2018packaging}:
...
\begin{quote}
``The R/ directory contains custom functions that are used repeatedly throughout the project. The man/ directory contains the manual (i.e., documentation) for the use of the functions.'' \citep{marwick2018packaging}
\end{quote}

Another project (workflowr) suggests the following subdirectories \citep{blischak2019creating}:
...
\item
  ...
  and other source code supporting the data analysis'' \citep{blischak2019creating}
\item
  data: ``for storing raw data files'' \citep{blischak2019creating}
\item
...
\item
  ...
  the scripts and analyses'' \citep{blischak2019creating}
\end{itemize}

They note that you want a directory setup that is ``flexible and configurable''
\citep{blischak2019creating}.
...
{[}Others not related to directory structure{]}.'' \citep{bertin2021creating}
\end{quote}
...
each subdirectory \citep{prodigenr}. That project incorporates the following
subdirectories:
...
Below a single experiment directory, the organization of files and directories is logical, and depends upon the structure of your experiment. In many simple experiments, you can keep all of your files in the current directory. If you start creating lots of files, then you should introduce some directory structure to store files of different types. This directory structure will typically be generated automatically from a driver script, as discussed below.'' \citep{noble2009quick}
\end{quote}
...
of the R packages your project depends on'' \citep{prodigenr}.
...
enough that the users can test it out \citep{osann2020design}.
...
\tightlist
\item
  Review the list of data you typically collect or files you create for that
  type of study or experiment, a list you created when designing the blueprint for
  the directory template
\item
  Create template files for any data collection that is typical for that type
  of study or experiment. Use example or placeholder data to create examples of
  those files.
\item
  Create a directory structure that divides the types of files into
  subdirectories of similar types.
\item
  Create one or more templates of report files that access and report on the
...
In this structure, we've selected names that are generic enough (e.g., ``data'',
...
this directory in the future, as the directories use unambiguous names.
...
a consistent name for each subdirectory when we include it.

\subsection{Step 3: Designing a report template}\label{step-3-designing-a-report-template}
...
\subsection{Step 4: Testing the project template and refining it}\label{step-4-testing-the-project-template-and-refining-it}
...
\subsection{Step 5: Using the project template}\label{step-5-using-the-project-template}
...
\begin{figure*}
\includegraphics[width=\textwidth]{figures/project_template_basic_use} \caption[Steps in using a basic project directory template that you have created for a type of study or experiment]{Steps in using a basic project directory template that you have created for a type of study or experiment.}\label{fig:basicprojecttemplateuse2}
\end{figure*}
...
As a research project progresses, researchers will often end up with many
files (e.g., `draft1.doc',
`draft2.doc'). This can result in
...
change research project files more cleanly, while including messages to explain
changes and maintaining the power to `backtrack' to previous versions. We
...
that you collect may also go through several versions. For example, if you identify
a typo in the collected data after you record it, you might correct the typo and
add a note or signature to explain that update. If you are collecting data at
multiple timepoints, you may have new versions of a data file as you complete
each timepoint.

When materials evolve across versions, it introduces challenges in maintaining a
research process that is smooth, efficient, and error-free. One challenge is to
maintain clarity and organization across different versions, so that it is
always clear which version is the most current, as well as which version should
be used for specific purposes. For example, if several coauthors are editing a
paper draft, it is important to ensure they are all working on the most recent
version.
...
will all work to edit the latest draft at the same time, rather than one-by-one.
...
as the document changes from version to version. This record can help in auditing
for errors or bugs that were introduced as the document evolved. The record will
...
which aims to record and document changes to the document or material over time,
coordinate the contributions of different members of a team as they change the
document or material, and ideally allow reverting back to older versions if
needed. In this module, we'll focus on version control as it applies to research
materials that are electronic files and directories, but you may also find it
useful to think about how the principles and elements of version control can be
applied to other research materials, like cell lines and antibodies.
...
The idea of \emph{control} is to allow for safe changes and updates to this version
while more than one person is working on it. Part of this ``control'' will also
include recording the changes made from one version to the next and annotating
reasons for those changes.

The general term \emph{version control} can refer to any method of syncing
contributions from several people to a file or set of files. While version
control of computer files can be done by people, and originally was
\citep{irving2011astonishments}, it's much more efficient to use a computer program
to handle this tracking and coordinate contributions from multiple people.

Software for this purpose---\emph{version control software}---first developed for
software programming projects, and some popular version control software today
...
in any implementation of version control.Later, we'll explain how some of these
...
repository is almost identical to a file directory (also called a file folder),
and indeed a repository starts from a file directory. The only difference is the
repository is enhanced with some additional overhead. This overhead is added to
record how the files in the directory have changed over time. You can compare
this to how you might track document changes if the documents were paper rather
than electronic---you could store the documents in a paper folder and add a
piece of paper where you record a log of each change you make to the documents
in the folder. The extra overhead that changes a regular file directory to a
repository is very similar to the log in this example. A repository, in other
words, is a directory that is under version control.

In a repository of files that is under version control, you take regular
``snapshots'' of how the files look during your work on them. Each snapshot is
called a \emph{commit}, and it provides a record of which lines in each file changed
from one snapshot to another, as well as exactly how they changed. The idea
behind these commits---recording the differences, line-by-line, between an older
and newer version of each file derives from a longstanding Unix command line
tool called \emph{diff}. This tool, developed early in the history of Unix at AT\&T
\citep{raymond2003art}, is a solid and well-tested tool that does the simple but
important job of generating a list of all the differences between two plain text
files. Each commit in a repository includes the same type of information about
the differences introduced in the files at the time of that commit.

When you are working with a directory under version control, you can also
explain your changes as you make them---in other words, version control allows
for \emph{annotation} of the developing and editing process \citep{raymondunderstanding}.
Each commit requires you to enter a \emph{commit message} describing why the changes
that are introduced with that commit were made. The commit messages can serve as
a powerful tool for explaining changes to other team members or for reminding
yourself in the future about why certain changes were made. A repository under
version control, then, includes a complete history of how files in a project
directory have changed over the timecourse of the project and why.

Further, each of the commits is given its own ID tag (in the \emph{git} software,
this is a unique SHA-1 hash), and version control systems have a number of
...
it at a time. A slightly more modern analogy is the idea of having a single version
...
every team members computer, which has the side benefit of provided natural
backup of the project files as we'll describe in more depth later in the module.
Remote repositories---which may be on a server in a different location---can be
added with another copy of the project, which can similarly be synced regularly
to update with any changes made to project files.
...
of version control through other platforms. For example, Google Docs enables
a level of version control through its Version History feature. This
feature allows you name different versions of a document that is saved in
Google Docs, restore a document to earlier versions, and see which changes
have been made to a document and who made each change.
...
the initial recording of data, but also keep a legal
...
multiple experiments \citep{giles2012digital, butler2005electronic, perkel2011coding}.
...
\citep{raymond2003art}. However, both git and GitHub can bring dramatic gains to
...
\emph{merging} the files. While this is typically described using the plural,
``files'', at a higher-level, you can think of this as just merging the \emph{changes}
...
list of what needs to be done to complete the project \citep{perez2016ten}, which are
...
Second, we'll briefly provide some information for scientists who \emph{are}
...
\url{https://github.com/}. Even if you are not coding, you will need to be logged in
...
ran the equipment\ldots{} What now?''.

For certain types of data, you may be able to proceed directly to analysis of
...
you will need to start with a stage of \emph{pre-processing} the data: that is,
...
complexity inherent in this type of high-dimensional data.

All in all, pre-processing aims to extract useful information from the measurments
...
that provide insights on your scientific question.

Let's start by talking about what data pre-processing is, as well as why
...
preprocessing before you incorporate it in a statistical analysis.
...
Data \emph{preprocessing} is a term that covers steps must be done after data are
...
correlated with the bacterial load in a sample.
...
several of the most common themes and process in data preprocessing.
...
this access is typically the first step in most data preprocessing pipelines.
...
For other assays, especially those that involve more complicated equipment, the
...
the mzML file format \citep{deutsch2012file}. Flow cytometers often output data in
the Flow Cytometry Standard (FCS) file format \citep{spidlen2021data}. In many cases,
...
into R from some of the specific file formats for biomedical data.
...
colleagues---which, unlike data generation, is not universally
...
technology tools capable of making sense of the vast data sets generated
by omics experiments. In fact, the development of such tools has become
a discipline unto itself, called bioinformatics. And only with those
tools can researchers hope to clear another obstacle to drug
development: that posed by so-called emergent properties---behaviors of
biological systems that cannot be predicted from the basic biochemical
...
\begin{marginfigure}
``One thing that has not changed in the last 10 years is that the
individual outputs of the sequence machines are essentially worthless by
themselves. \ldots{} Fundamental to creating biological understanding
from the increasing piles of sequence data is the development of
analysis algorithms able to assess the success of the experiments and
synthesize the data into manageable and understandable pieces.''

{[}@flicek2009sense{]}
\end{marginfigure}

The discipline of bioinformatics works to develop these types of algorithms
\citep{barry2009new}, and many of them are available through open-source, scripted
...
\textbf{Addressing technical noise}
...
process, however, varies across cells \citep{perkel2017single}. If this isn't
addressed in preprocessing, then this ``amplification bias'' prevents any
meaningful comparison across cells.

\begin{quote}
``In many cases\ldots{} the tools used in bulk RNA-seq can be applied to scRNA-seq.
But fundamental differences in the data mean that this is not always possible.
For one thing, single-cell data are noisier\ldots{} With so little RNA to work with,
...
picked up in the sequence data. Another challenge is the scale\ldots{} A typical bulk
...
when confronted with ten or a hundred times as many.'' \citep{perkel2017single}
\end{quote}
...
increased technical variability.'' \citep{bacher2017scnorm}
\end{quote}
...
accounted for. One way to adjust for this is \ldots{} {[}Sequencing depth / read depth
...
present within the data set.'' \citep{lytal2020normalization}
\end{quote}
...
scale factor \citep{bacher2017scnorm}.
...
more than others \citep{haque2017practical}.
...
and noise correction \citep{luecken2019current}.
...
mean and unit vari- ance (z scores). \ldots{} There is currently no consensus on
whether or not to perform normalization over genes.'' \citep{luecken2019current}
\end{quote}
...
Another application of the logarithmic transformation is \ldots{} to transform a
severely skewed distribution to normality.'' \citep{chatfield1995problem}
\end{quote}
...
level of tens of thousands of different genes \citep{perkel2017single}. Data
...
datasets with more columns than rows \citep{chatfield1995problem}.
...
t-distributed stochastic neighbor embedding (t-SNE) \citep{perkel2017single}. Newer
...
on their gene expression profiles, through scRNA-seq \citep{haque2017practical}.
...
often requires special consideration.'' \citep{haque2017practical}
\end{quote}
...
minimum. \ldots{}'' {[}\citet{chatfield1995problem}
\end{quote}
...
R \citep{perkel2017single}, and noted that this was a barrier to using many of the
...
in by-hand processing \citep{gibb2014reproducibility}.
...
Transparency is a key element of reproducibility \citep{neff2021past}. If the
...
While you only need to \emph{install} a package once, you need to \emph{load} it every
...
will be a complaint.'' \citep{kernighan2011d}
\end{quote}
...
\subsection{Tips and guidelines for writing code scripts}\label{tips-and-guidelines-for-writing-code-scripts}
...
\textbf{Write code for computers, rewrite it for humans}
...
object names, and they even have a fancy name, \emph{metasyntactic variables}.
...
{[}More on the importance of learning a few foundational tools well{]}
...
In later modules, we will recommend some of the tools that we have found to be
...
regardless of the programming language you are learning.
...
scholar Brian Arthur calls \emph{deep craft}---the ability to know intimately
...
airplanes have a strong connection to deep craft.'' \citep{madhavan2015applied}
\end{quote}
...
\ldots{} A specific technique in modular systems thinking, for example, includes
...
engineer useful outcomes. \ldots{} there's a top-down design strategy---`divide
and conquer'---in which each subtask is separately attacked in a progression
toward achieving the final objective.'' \citep{madhavan2015applied}
\end{quote}

\begin{quote}
``Psychologist Gary Bradshaw has written about the importance of function-based
...
measurement techniques.'' \citep{madhavan2015applied}
\end{quote}
...
use a standard tool that you've used before, like the \texttt{readr} function \texttt{read\_csv},
...
the starting points toward our ultimate creative destinations.'' \citep{madhavan2015applied}
\end{quote}

\begin{quote}
``Tweaking and prototyping are basic human habits.'' \citep{madhavan2015applied}
\end{quote}
...
\tightlist
\item
  Faceting in \texttt{ggplot2}
\item
  The \texttt{map} family of functions in the \texttt{purrr} package
\item
  The \texttt{group\_by} and \texttt{summarize} / \texttt{mutate} workflow in the \texttt{dplyr} package
\end{itemize}
...
through the whole script once you have built your pipeline.

Use comments! In R, and line that starts with a ``\#'' will be ignored by R, so you
...
to more information on that function.

You can also clean up the code substantially by editing it to follow a set of
...
one created by Hadley Wickham available at: \url{https://style.tidyverse.org/index.html}.
...
edith the footage\ldots{} Why produce a lot of work and then throw away 95 percent?
The two processes seem contradictory. Why not just produce the 5 percent ultimately
used? Because you don't know \emph{which} 5 percent that will be. Editing can
be hard becuase you're discarding things you have put a lot of energy into
making.'' \citep{judkins2016art}
\end{quote}

\begin{quote}
``You might not write well every day, but you can always edit a bad page.
You can't edit a blank page.'' -- Jodi Picoult, cited in \citep{judkins2016art}
\end{quote}
...
need to run \texttt{install.package} function calls everytime you run the code in a
...
from packages, it will include the code to load those packages (\texttt{library}
...
\subsection{Potential quotes}\label{potential-quotes-1}
...
a coast-to-coast railway system within the decade. \ldots{} Throughout his
...
at 300 points in the country.' \ldots{} Even regionally, timekeeping was in
...
when trains operated on a single track.'' \citep{madhavan2015applied}
\end{quote}

\begin{quote}
``Simplicity is not about stripping features down to a bare minimum.
It's about achieving elegance while maintaining performance.'' \citep{madhavan2015applied}
\end{quote}

\begin{quote}
``The essence of good technology is that it's intuitive, and it evolves.
Ideally, you don't even want to know it's there.'' \citep{madhavan2015applied}
\end{quote}
...
Since the functions in the tidyverse approach are designed to work in a modular
...
combining sets of simple functions in interesting ways.
...
To be able to work with data in R, you first must load it into your R
...
\textbf{Tools for changing or creating columns}
...
differences based on the units used for measurements.

There are also more complex methods for scaling and normalization. All similarly
...
conversion can easily be done using functions from the \texttt{dplyr} package. They are
also critical for performing processes like scaling / normalization--- the
\texttt{mutate} function, for example, can be used to create a new column of scaled
data by applying a scaling function to an existing column.

The package includes not only functions for making changes to a single column
(e.g., the \texttt{mutate} function), but also functions that can be used to perform
the same calculation across many columns (e.g., the \texttt{across} function). This
...
off the street to help create new things \citep{judkins2016art}. An open
...
data are restriction and compression. Restriction means controlling which data
are loaded and lets us avoid wasting resources on irrelevant or excessive data.
Compression helps by representing the same data with fewer resources.'' \citep{lawrence2014scalable}
\end{quote}
...
\begin{quote}
``There is a cost to the free lunch. That \texttt{print} is generic means that what
