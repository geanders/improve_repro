...
original sample based on the colonies counted at this dilution.
...
RStudio. The data are already in a ``tidy'' data format, as described in module {[}x{]}.
Each row represents \ldots{} . Columns are included with values for \ldots{} and measurements
of \ldots{} for each row.

The final pre-processing protocol for these data can be downloaded, including both
the original RMarkdown file ({[}file link{]}) and the output PDF document ({[}file link{]}).
