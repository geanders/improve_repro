...
toward a different style, allowing for \emph{distributed} rather than \emph{centralized}
...
working, including \emph{branching} to try out new ideas that you're not sure you'll
ultimately want to go with and \emph{forking}, a key tool used in open-source software
...
explain your changes as you make them---in other words, it allows for \emph{annotation}
...
help, \emph{commit} those changes with a message (a \emph{commit message}) about why they think
...
Because the changes made to the files in the project are regularly \emph{committed}, with a
message for each commit and a record of what was changed in that commit, the team members can,
at any point, go back to the \emph{exact} version of the project at any earlier point in time.
Each of the commits is given its own ID tag ({[}name for this? hash?{]}), and \emph{git} has a number
...
\textbf{don't} need to save earlier versions of each file. You can maintain one and only one
