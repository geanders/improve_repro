...
allows for \emph{annotation} of the developing and editing process
\citep{raymondunderstanding}. The ability in version control software to compare
differences between two plain text files derives from a longstanding Unix
command line tool called \texttt{diff}. This tool developed early in the history of
Unix at AT\&T \citep{raymond2003art}. This small, extremely solid and well-tested tool
...
Each snapshot of changes to the files in a directory is called a \emph{commit},
which is tagged with an identifier so it can be found later, as well as a
\emph{commit message} describing why the change was made. The commit messages
...
\citep{perkel2018git, blischak2016quick}.
