...
created using the package `bookdown`. There is an online book available with
...
To learn how to create websites with blogs, you can check the book *blogdown*
...
documents. The package is called `kableExtra`. You can install it the same as
any other R package from CRAN, using `install.packages`. You will need to use
then need to use `library("kableExtra)` within your RMarkdown document before
...
The `kableExtra` package is extensively documented through two vignettes that
...
`kableExtra` available through [RMarkdown book. doublecheck which one.
Cookbook?]
