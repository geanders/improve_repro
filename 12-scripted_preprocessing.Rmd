...
time-consuming for repeated tasks. We will explain how preprocessing can be
broken into common themes and processes, as well as how scripted pre-processing,
especially using open source software, can improve transparency and
reproducibility.
...
We'll start this module by covering what preprocessing is, and then common
reasons for preprocessing and common steps in the process. Later in the module,
we'll introduce the idea of using scripted code to preprocess data, an idea that
we'll cover in much more detail in the next few modules. We'll finish the module
with a couple of examples of the steps in specific preprocessing pipelines, 
along with a chance for discussion of how these steps align with common themes
in preprocessing. 
...
In the research process, pre-processing steps must be done before the data are
used for further analysis. There are the first step in working with the data
after they are collected by the equipment (or by laboratory personal, in the
case of data from simpler process, like plating samples and counting
colony-forming units). After the data are appropriately pre-processed, you can
use them for statistical tests---for example, to determine if metabolite
profiles are different between experimental groups---and also combine them with
other data collected from the experiment---for example, to see whether certain
metabolite levels are correlated with the bacterial load in a sample.

It is a worthwhile investment of time to learn
...
Let's start by talking about why biomedical data often need extensive
preprocessing. When you run an assay, you are aiming to gain knowledge and
insights on a scientific question. For example, does a new drug help in curing a
disease? There are some measures that you can take that are directly linked to
the scientific question. For example, if you track people who take a drug versus
a placebo for a terminal disease, the length of time that each subject survives
would be a direct measure of the success of that drug. When you have direct
measurements like this, your data may not require much preprocessing before you
incorporate it in a statistical analysis.

However, there are often cases where we collect data that are not as immediately
linked to the scientific question. Instead, these data may require
pre-processing before they can be used to test meaningful scientific hypotheses.
This is often the case for data extracted using complex equipment. Equipment
like mass spectrometers and flow cytometers leverage physics, chemistry, and
biology in clever ways to help us derive more information from samples, but one
tradeoff is that the data from such equipment often require a bit of work to
move into a format that is useful for answering scientific questions.
...
reduce or handle noise and bias in the data collection. 
...
data, across types of data collection and research questions. These common
...
To be able to work with data---whether to preprocess it or to analyze it---you
...
of the method that the software needs to access the data, it will need some kind
of access. Therefore, the first step in most data preprocessing pipelines is to
provide the software access to the data.

When working with biomedical research data, you will typically have it stored in
...
you enter and save in the Excel program may be saved to an ".xls" or ".xlsx" file
...
It is usually very straightforward to read data from these simper file formats 
into R and other similar software. In later modules, we'll provide examples
of how to read in from these types of file formats. 

For other assays, especially those that involve more complicated equipment, 
the file format will be more complex than a basic spreadsheet. For example, 
mass spectrometers, which are used to collect data for proteomics and 
metabolomics, often output data in [file format]. Flow cytometers often 
output data in [file format]. In many cases, these file formats will be 
well-defined and standardized across equipment, which makes it easier for
open source developers to build functions to input data from those file 
formats. 

Often, the initial download of R or similar software will not include tools
...
[Where to find resources for more complex file formats]

**Extracting scientifically-relevant measurement**

Another common purpose of preprocessing is to translate the measurements that
...
exact process that was used for that ELISA (e.g., sandwich vs. ...).
...
However, they all require steps to translate the data that are immediately
recorded or read by equipment into data that are more scientifically meaningful.
One example is flow cytometry, which can characterize immune cell populations 
through a process that leverages the physics of lasers and fluorescence, as well
as the biology of antibody specificity [@benoist2011flow]. 
...
data (intensity of light at different wavelengths) to back-calculate from that
measure to a measure of which of the proteins on the panel are present in the
cell. To interpret the data that are recorded by a flow cytometer---and to
generate a characterization of immune cell populations from these data---you
need to incorporate a number of steps of translation. These include steps that
...
[@barry2009new], and many of them are available through open-source, scripted
...
manufacturers and sometimes separately. [Example: FlowJo]
...
research data is through basic exploratory analysis. Basic dot plots or
...
example, if most recorded mouse weights are around [weight], but one is recorded
as [x], you may be able to identify that the recorder missed a decimal point 
...
```{marginfigure, echo = TRUE}
"It is important to
find out why an observation is missing. This is best done by asking 'people in
the field'. In particular, there is a world of difference between observations
that are lost through random events, and situations where missing observations
due to damage or loss are more likely to arise under certain conditions."
...
**Addressing technical noise and bias**

The second common purpose of preprocessing data is to address, to the extent
...
use to answer scientific questions. 

> "Statistical models usually contain one or more ssytematic components as well
...
are in their growth? cycle. However, if you are trying to use single cell
...
researchers are working to weigh the mice for an experiment, the recorded
weights recorded by one of the researcher might tend to be, on average, lower
...
These batch effects can often be corrected for through statistical modeling, 
...
include much more high-throughput data. These data often include measurements
for each sample on hundreds of thousands of different parameters. For example,
gene expression data can include measurements for each sample on the expression
level of tens of thousands of different genes [@perkel2017single], metabolics
data can include measurements for thousands [?] of metabolite features,
proteomics can include measurements for thousands [?] of proteins, and so on.
...
for each obsevation. If you envision a dataset as a two-dimensional table, with
the observations as the rows and the characteristics measured for each
observation as the columns, then high-dimensional data are large in terms of the
number of columns in the table. A growing number of biomedical data now also are
large in the number of observations they capture---in the number of rows in the
table of data, in other words. A longer-standing example of this is flow
cytometry data, where the observations are individual cells. Current experiments
often capture in the range of [x] cells for each sample in flow cytometry,
measuring for each characteristics that can be used to determine cell size,
granularity, and surface proteins, all with the aim of characterizing the cells
by cell type. A more recent example is with single cell RNA-sequencing. Again,
with this technique, observations are taken at the level of the cell, with on
the order of [x] cells processed per sample.

Whether data is large because it measures many features (high throughput data)
...
For these types of high throughput data, the different measurements often 
...
Dimension reduction techniques---principal components analysis (PCA) is one very
popular one---is therefore often a key element of data preprocessing when
working with high-dimensional biological data. Dimension reduction helps to
collect the information that is captured by the dataset into fewer columns, or
"dimensions"---to go, for instance, from columns that measure the expression of
thousands of different genes down to a few "principal component" columns that
capture the key sources of variation across these genes. One long-standing
approach to dimension reduction is principal components analysis (PCA). Other
newer techniques have been developed, as well, such as t-distributed stochastic
neighbor embedding (t-SNE) [@perkel2017single]. Newer techniques often aim to
improve on limitations of classic techniques like PCA under the conditions of
current biomedical data---for example, some may help address problems that arise
when applying dimension reduction techniques to very large datasets [?].

To get a basic idea of dimension reduction, consider this example. Say you have
conducted an experiment that includes two species of research mice, C57 black 6
and BALB/C. You record information about each mouse, including columns that
record both which species the mouse is and what color its coat is. Since C57
black 6 mice are always black, and BALB/C mice are always white, these two
columns of data will be perfectly correlated. Therefore, one of the two columns
adds no information---once you have one of the measurements for a mouse, you can
perfectly deduce what the other measurement will be. You could therefore,
without any lose of information, reduce the dimensions (number of columns) of
the data you've collected in this case by choosing only one of these two columns
to keep and getting rid of the other column.
...
> "Multivariate techniques may be used to reduce the dimensionality... It is
potentially dangerous to allow the number of variables to exceed the number of
observations because of non-uniqueness and singularity problems. Put simply, the
unwary analyst may try to estimate more parameters than there are observations."
[@chatfield1995problem]

> "Principal component analysis rotates the p observed values to p new
orthogonal variables, called principal components, which are linear combinations
of the original variables and are chosen in turn to explain as much of the
variation as possible. It is sometimes possible to confine attention to the
first two or three components, which reduces the effective dimensionality of the
problem. In particular, a scatter diagram of the first two components is often
helpful in detecting clusters of individuals or outliers."
[@chatfield1995problem]
...
cells [?], removing the "housekeeping" genes with similar expression regardless
of the cell type. 

> "For each gene, QC metrics such as the average expression level and the
proportion of cells in which the gene is expressed are computed. This can be
used to identify low-abundance genes or genes with high dropout rates that
should be filtered out prior to downstream analyses." [@mccarthy2017scater]

> "These approaches aim to identify, for a given dataset, a subset of features (for example, transcripts) that are meaningful or informative. These are signatures (that is, subsets of analytes detected by a given assay) that differentiate study groups, or that correlate with other analytes or study parameters of interest. Feature selection relies on arbitrary cut-offs that may be statistical (p values) or otherwise (for example, fold change). An alternative to the use of cut-offs is the rank ordering of analytes (for example, on the basis of fold change, r values, p values or other parameters)." [@chaussabel2014democratizing]
...
cell in a sample of what can be [thousands?] of cells. One goal of scRNA-seq is
to use gene expression patterns in each cell to identify distinct cell types in
the sample, potentially include cell types that were not known prior to the
...
### Tools for biomedical data preprocessing

The previous parts of the module described some common themes and processes in 
...
While we've covered some key processes of preprocessing, we haven't talked 
yet about the tools you can use to implement it. Most preprocessing will be 
done on the computer, with software tools. An exception might be for very 
simple preprocessing tasks---one example is generating the average cage weight
for a group of mice based on the total cage weight and the number of mice. 
However, even these simple processes can be done with a computer, and doing 
so can help avoid errors and to provide a record of the calculation that was 
used for the preprocessing. 

You will have a choice about which type of software you use for preprocessing.
There are two key dimensions that separate these choices---whether the software
is point-and-click software versus script-based software, and whether the
software is proprietary versus open-source software. It is important to note
...
In this section of the module, we'll talk about the advantages and disadvantages of these different types of software. For reproducibility and rigor, there
are many advantages to using software that is script-based and open source for
data preprocessing, and so in later modules, we'll provide more information on 
how you can use this type of software for preprocessing biomedical data.

**GUI versus code script**

When you pick software for preprocessing, the first key dimension to consider
is whether the software is point-and-click software or script-based software. 
...
on your mouse most of the time, and you use the mouse to select actions and
options from buttons and other widgets [?] that are shown by the software on the
screen. This type of software is sometimes called "widget-based", as it is built
around widgets like drop-down menus and slider bars [@perkel2018future].
...
script-based software, you keep your keys on the keyboard more often than on the
mouse. Many script-based software programs will also allow you to also send the
lines of instructions one at a time in an area called a "console", which will
then return the result from each line after you run it. Script-based software is
also sometimes called software that is "used programatically"
[@perkel2018future]. Script-based software in common use with biomedical 
data include R, Python, and Unix bash scripts, as well as some less common 
...
While we ultimately recommend script-based software, there are some appealing
features of point-and-click software. These features likely contribute to 
its wide popularity and to the fact that the vase majority of software that you 
use in your day-to-day life is now point-and-click.
...
provide a much smaller set of options that a full programming language does.

By contrast, coding languages take more investment of time and energy to
initially learn how to use that many GUI-based software programs. This is
becuase a coding language is just that---a language. It is built on a set (often
...
scientist to become comfortable using a script-based software like R or Python.
With another year or so of regular use, scientists can often start making their
own small software extensions to the language. In a 2017 article on analyzing
scRNA-seq data, the author noted that "relatively few biologists are comfortable working in those environments", referring to Unix and R [@perkel2017single], 
and noted that this was a barrier to using many of the available tools for
working with scRNA-seq data at the time. However, we would argue that 
becoming comfortable with these tools requires an investment, but is not
an insurmountable barrier for biologists. 

It is true that this is a substantially larger investment in training than a
short course or workshop, which might be adequate for learning to start with
...
> "most biomedical researchers experience great difficulty carrying out
computationally complex analyses on large data sets. " [@nekrutenko2012next]
...
you pressed when using a GUI, it's very easy to forget to record a step. When
you use a code script, it will not run if you forget a step or a detail of that
step. Some GUI-based programs are taking steps to try to ameliorate this,
allowing a user to save or download a full record that records the steps taken
in a given pipeline [more on this?]. As a note, there are some movements towards
"integrative frameworks", which can help improve reproducibility for pipelines
that span different types of software [?] (Galaxy, Gene Prof).

> "The above challenges can be distilled into two main issues. First, most
biomedical researchers experience great difficulty carrying out computationally
complex analyses on large data sets. Second, there is a lack of mechanism for
documenting analytical steps in detail. Recently, a number of solutions have
been developed that, if widely adopted, would solve the bulk of these
challenges. These solutions can collectively be called integrative frameworks,
as they bring together diverse tools under the umbrella of a unified interface.
These include BioExtract, Galaxy, GenePattern, Gene Prof, Mobyle, and others.
These tools record all analysis metadata, including tools, versions and
parameter settings used, ultimately transforming research provenance for a task
that requires active tracking by the analyst to one that is completely
automatic." [@nekrutenko2012next]
...
often are easier to automate than a workflow through a GUI-based system. For
...
kind of automation can also help in limited errors from human mistakes
in by-hand processing [@gibb2014reproducibility].

> "Another approach to maximizing reproducibility is to use automation to cut
out sloppy, all-too-human mistakes." [@gibb2014reproducibility]


*Examples of GUIs for data preprocessing*: FlowJo, ...

Some GUI-based programs are proprietary and some are open-source. Examples 
of GUI-based programs for scRNA-seq preprocessing and analysis include
SeqGeq (proprietary), Ganatum (open source) and the Automated Single-cell
Analysis Pipeline (ASAP, also open source) [@perkel2017single].
...
fix it, and you need replacement parts." [@johnson2011good]

Many scripted languages are based on a Unix-style philosophy of creating small,
simple tools that can be linked together in powerful ways. In this case, each
tools (in R, this would be a function) might do something very simple on its
own, but because you can "pipe" from one tool to another, you can combine these
small tools into pipelines that do very powerful and complex tasks. In this
context, a "pipe" allows you to take the output from one tool and send it
directly into another (for example, in R, to send the output from one function
call directly into another function) [@kernighan1984unix]. A "pipeline", then,
is a linked set of these tools, linked with pipes to connect the output from one
as the input to the next [@kernighan1984unix].
...
The other dimension of software for preprocessing is whether it is open-source
or proprietary. Open-source software is software where you can access, explore, 
and build on all the underlying code for the software. By contrast, the code
that powers proprietary software is typically kept private, so you can use the
product but cannot explore the way that it is built or extend it in the same 
way that you can open-source software. 
...
rule, and there are examples of open-source point-and-click software (e.g., ...)
as well as proprietary script-based software (e.g., Matlab [?]).

There are advantages and disadvantages to both types of software, but in terms
of rigor and reproducibility, open-source software often has the advantage.

One facet where proprietary software has an advantage is that it will often have
more comprehensive user support than open-source software. The companies that
make and sell proprietary software will have a user support team to answer
questions and help develop pipelines.

Some open-source software also has robust user support. In some cases, this has
developed as a result of a large community of users who help each other. 
Message boards [?] like StackOverflow provides a forum for users to ask 
and respond to questions. Some companies also exist that provide, as their 
business model, user support for open-source software. While open-source 
software is usually free, these companies make money by providing support
for that software. 
...
software, where there is often not a helpline and so the community of users
...
Often, for open-source scientific software the algorithms and their principles
have gone through peer review as part of the academic publication process.
...
the details of how the underlying algorithms work. Also, the algorithms may not
have gone through peer-review, if they are considered private intellectual
property.

> " Several commercial vendors now provide 'software as a service' analysis
solutions for sequence data (such as DNAnexus and GenomeQuest); however, these
solutions are built on proprietary platforms, raising concerns about
transparency and vendor lock-in." [@nekrutenko2012next]

Transparency is a key element of reproducibility. If the algorithms of software
can be investigated, then scientists who are using two different programs (for
example, one program in Python and one in R) can determine if their choice of
program is causing differences in their results. By contrast, if two research
groups use two different types of proprietary software, the algorithms that
underlie the processing are often kept secret and so cannot be compared. In that
case, if the two groups conduct the same experiment and get different results,
it's impossible to rule out whether the difference was caused by the choice of
software.
...
With open-source, you can think of it as builing a collection of different
tools, rather than a black box. The tools can come from different companies
(like real tools)---you don't have to limit to consuming the tools created by a
single producer (as with most proprietary software systems). For free and
open-source software, you don't have to invest money to get more tools. Instead,
the investment for each new tool is the time that it takes to learn how to use
it in your workflow.
...
groupings of the experimental units) and so on." [@chatfield1995problem]
...
observations (called influential observations)." [@chatfield1995problem] 

> "Missing observations arise for a variety of reasons. ... It is important to find out
...
more likely to arise under certain conditions." [@chatfield1995problem]
...
of their components." [@barry2009new]
...
> "Although each [scRNA-seq] experiment is unique ..., most analysis pipelines follow
...
functions." [@perkel2017single]

> "In many cases... the tools used in bulk RNA-seq can be applied to scRNA-seq. But 
...
times as many." [@perkel2017single]
...
because a program thinks there is insufficient RNA for processing." [@perkel2017single]
...
and the integration of protein-interaction data as well." [@perkel2017single]
...
preserve information on cellular origin." [@haque2017practical]
...
cellular origin." [@haque2017practical]
...
tools similar to those used for bulk samples." [@haque2017practical]

> "More recently, droplet-based platforms ... have become commercially available...
...
isolation through flow-cytometric sorting or micro-dissection." [@haque2017practical]
...
raw sequencing data." [@mccarthy2017scater]
...
within an R programming environment." [@mccarthy2017scater]
...
identify these low quality cells [@ilicic2016classification].
...
of the data and therefore need to be excluded." [@ilicic2016classification]
...
and high quality cells." [@ilicic2016classification]

> "Genes related to mtDNA were upregulated in low quality cells... This suggested 
that these cells are broken and thus of low quality." [@ilicic2016classification]
...
quality cells." [@ilicic2016classification]

> "At least a subset of the features considered are cell type specific. ... 
...
lowly expressed are highly cell type specific." [@ilicic2016classification]
...
mitochondria will be retained, thus explaining our observation." [@ilicic2016classification]
...
of the categories." [@ilicic2016classification]
...
using barcode sequence information." [@haque2017practical]

> "Dropout: An event in which a transcript is not detected in the sequencing
data owing to a failure to capture or amplify it." [@haque2017practical]

> "Sequencing depth: A measure of the sequencing capacity spent on a single sample, 
reported for example as the number of raw reads per cell." [@haque2017practical]

> "Spike-in: A molecule or set of molecules introduced to the sample in order to 
calibrate measurements and account for technical variation" [@haque2017practical]
...
periods, resulting in temporal fluctuation of transcript levels." [@haque2017practical]
...
for amplification biases." [@haque2017practical]
...
cell in processing that renders it unsuitable for downstream analyses." [@mccarthy2017scater]
...
captured during library preparation and are detected at greater frequency." [@mccarthy2017scater]
...
levels of constitutively expressed genes like ACTB." [@mccarthy2017scater]
...
- technical size factor with spike-ins [@brennecke2013accounting], plus a 
biological size factor [?]

> "Progress in gene expression analysis using minute amounts of starting 
material has made single-cell transcriptomics accessible." [@brennecke2013accounting]
...
weak to strong." [@brennecke2013accounting]
...
the same large number of genes, but in a very different matter." [@haque2017practical]
...
therefore improve precision." [@haque2017practical]
...
exact relatinoships are protocol specific." [@haque2017practical]
...
genes) are likely to perform suboptimally on scRNA-seq data." [@haque2017practical]
...
provide normalized estimates of expression." [@bacher2017scnorm]
...
be near one... since the cells between the two groups are identical." [@bacher2017scnorm]
...
ensure that results are meaningful." [@bacher2017scnorm]
...
and therefore lose their effectiveness when applied to scRNA-seq." [@lytal2020normalization]
...
processing." [@lytal2020normalization]
...
comparative sparsity of scRNA-seq data." [@hafemeister2019normalization]
...
for genuine differences in biological expression levels." [@brennecke2013accounting]
...
population under study." [@brennecke2013accounting]
...
identifying differentially expressed genes across cell populations." [@haque2017practical]
...
runners from the air, Trapnell says." [@perkel2017single]

> "Other tools address subpopulation detection ... and spatial positioning, which uses
data on the distribution of gene expression in tissues to determine where in a 
tissue  each transcriptome arose." [@perkel2017single]
...
best determinant of overall cell function." [@lakhani2001microarray]
...
as a central technology for the field of proteomics." [@steen2004abc]
...
modifications in an approach called 'top-down' protein sequencing." [@steen2004abc]
...
unstable or insoluble under the conditions used." [@steen2004abc]
...
'electrospray ionization'." [@steen2004abc]
...
thousands of peptides are being identified." [@steen2004abc]
...
decreases if no other peptides do so." [@steen2004abc]
...
from such global experiments." [@steen2004abc]
...
calculate the molecular weight of a particular peptide." [@steen2004abc]
[@steen2004abc]
...
a homogeneous population, are now split into more than 10 subsets." [@benoist2011flow]
...
simplified gating technologies." [@barnett2008cd4]
...
differentiation stage." [@seder2008t]
...
fraction of the total response." [@seder2008t]
...
subsets." [@maecker2012standardizing]
...
becomes crucial." [@maecker2012standardizing]
...
the knowledge of immune function that they have contributed." [@maecker2012standardizing]
...
reperotoires, among other parameters." [@maecker2012standardizing]
...
> [@maecker2012standardizing] recommend, as ways to minimize the effects of technical 
...
influence results." [@maecker2012standardizing]
...
of dim particles over a series of voltage gains." [@maecker2012standardizing]
...
manual gating." [@maecker2012standardizing]
...
of immune responses." [@benoist2011flow]
...
data than an array, is largely at the beginning stages [as of 2011]." [@mak2011john]
...
normalization---becomes much more important." [@mak2011john]
...
Cambridge, bulk RNA-seq is 'pretty much a solved problem'." [@perkel2017single]
...
others, creating an alarming reproducibility crisis." [@nekrutenko2012next]
...
co-authored by at least one member of the 1000 Genomes Consortium." [@nekrutenko2012next]
...
components." [@raphael2020controlled]
...
computations. Now that we do, it seems logical to rethink this." [@neff2021past]
...
findings are reproducible." [@lithgow2017long]
...
you to do the same." [@stark2018before]
...
cancer research papers could not be reproduced." [@baker2015blame]
