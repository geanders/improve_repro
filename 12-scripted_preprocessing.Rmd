...
### Why do data need preprocessing?
...
### Common steps in data preprocessing for biomedical research

**Data input**

When working with biomedical research data, you will typically have it 
stored in some type of computer file. Computer files can have different 
file formats, which define for the computer the way that the data are 
stored in the file. For example, data that you enter and save in the Excel
program may be saved to an "xls" or "xlsx" file format [?]. Some types of 
equipment, like plate readers and [?], may also allow you to output data 
in this file format, or in a simpler plain text format like a comma-separated
file ("csv") format.

To be able to work with data in a program like R or Python, you'll need to 
first load the data into the program. This means you will need to direct the
program in how to access and load the data from within whatever file format
it's in.

Fortunately, there are a variety of functions, both in base R and in free
extension packages, that can read in data from a variety of file formats. 
Loading data with one of these functions will often be the first step in 
a data pre-processing pipeline. 

**Quality assessment / quality control**

Once data are loaded, a common next step is to check the data. 

When data are recorded by hand (for example, into a spreadsheet), there is the
possibility for recording errors. For example, when recording the weights of
mice, someone might transpose two numbers, so that a weight of [x] is recorded
as [x].

When data are recorded and saved by equipment, it typically avoids recording 
errors. However, there is still the possibility for data quality issues. 
[...]

Further, there can be patterns in the data that result from the way that the
...
One way you can help to identify potential quality issues in research data is
through basic exploratory analysis. Basic dot plots or scatterplots of the data,
for example, can often help to identify unusual outliers, which may be the
result of a data recording error or an oddity in one of the samples.
...
In some cases, some of the required data preprocessing will aim to apply 
scientific principles to extract helpful information from the data that are
returned by the scientific equipment. 

As one example, consider flow cytometry. This is a method of identifying 
different types of cells within a sample [?]. It works by ...
...
level of thousands [?] of different genes, metabolics data can include 
measurements for thousands [?] of metabolite features, proteomics can 
include measurements for thousands [?] of proteins, and so on. 
...
To handle the complexity and size of high throughput data, it can be helpful 
to try to either reduce the dimensions of the data, through a technique like
principal components analysis, or to use the high dimensional data to cluster
the data into groups of samples with similar patterns across the measurements. 

[dimension reduction]

[clustering]

### Examples of biomedical data preprocessing
...
skewed distribution to normality." [@chatfield1995problem]
