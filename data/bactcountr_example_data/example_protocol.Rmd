...
First, we can calculate the actual dilution for each test tube used in the
dilutions by taking the dilution factor (`dilution_factor`) and raising it to
the power of the dilution level (`dilution`). For example, the dilution at level
2 should be $0.5^2 = 0.25$, while the dilution at level 0 (undiluted) should be
$0.5^0 = 1$).

```{r}
cfu_data_reduced <- cfu_data_reduced %>% 
  mutate(dilution_in_tube = dilution_factor^dilution)

cfu_data_reduced
```

Next, we can determine the dilution on each of the *plates* by combining
information about the dilution in the test tube used for that plate and the 
volume taken from that test tube for plating: 

```{r}
cfu_data_reduced <- cfu_data_reduced %>% 
  mutate(dilution_on_plate = dilution_in_tube / (volume_plated / 1000))
...
cfu_data_reduced <- cfu_data_reduced %>% 
  mutate(bact_load_in_sample = CFUs * 
           dilution_on_plate * volume_resuspend)
cfu_data_reduced
```
