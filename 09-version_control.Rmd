...
- Define “versions” and “version control”  
- Identify examples of versioning both in a digital context (data, code, files) and for non-digital needs (protocols, lot numbers, intra lab standards) 
- Describe strategies and tools to save and refer to specific versions of data and other files or directories 
...
researchers, rather than an individual scientist working along. Collaboration
can drive excellent science, but it also creates additional challenges. One such
...
that you collect may go through several versions---for example, if you identify
...
When materials that evolve in this way across different versions, it introduces
challenges in maintaining a research process that is smooth, efficient, and
error-free. One challenge is to maintain clarity and organization across
different versions, so that it is always clear which version is the most
current, as well as which version should be used for specific purposes. For
example, if several coauthors are editing a paper draft, it is important to
ensure they are all working on the most recent version.

Another challenge is to coordinate the changes that different people make if they
work on the material at the same time. Scientific collaboration often does not
operate as an assembly line, where one person finishes their work on a document
or material and then hands it off to the next person. Instead, there will often 
be several copies of a version in different peoples' hands, with all of them 
working on it at once. The example of a paper draft is helpful here, too---often
coauthors will all work to edit the latest draft of a paper at the same time, 
rather than one-by-one. This creates the challenge of taking the contributions of 
each person and coordinating their changes and additions into one primary copy.
...
applied to other research materials.
...
contributions from several people to a file or set of files, and very early on
it was done by people rather than through a computer program. While version
...
to handle this tracking of the history of a set of files as they evolve.
*Version control software* developed as a way to coordinate collaborative work on
programming projects. In this section, we'll introduce the key features of
version control using examples and terminology from a common version control
software program called *git*. While these terms are derived from this
particular software program, they represent ideas that are important in any
...
Version control software can be used to track electronic files. While the very
earliest version control software systems tracked single files, these systems
quickly moved to tracking sets of files, called *repositories*. A repository is
almost identical to a file directory (also called a file folder), and starts
from a file directory. The only difference is the repository is enhanced with
some additional overhead that is added to record how the files in the directory
have changed over time. This means that a repository is a directory that is
under version control. You can compare this to how you might track document
changes if the documents were paper rather than electronic---you could store the
documents in a paper folder and add a piece of paper that serves as a log of
changes, using that log to record each change you make to the other documents in
the folder. The extra overhead that changes a regular file directory to a
repository is very similar to the log in this example.

In a repository of
files that is under version control, you take regular "snapshots" of how the
files look during your work on them. Each snapshot is called a
...
a level of version control through its Version history feature.
...
the initial recording of data, but also can serve as a legal
...
may lack adequate flexibility to handle the complex experiments often conducted.
...
adopted platform for electronic laboratory notebooks has yet to be taken up by
the scientific community [@kwok2018lab], despite clear advantages of recording
data directly into a computer rather than first using a paper notebook.
...
small subset of its basic functionality [@perez2016ten]. In module 2.11, we
