...
how data is recorded and communicated. Standards can clarify both the *content*
...
describe and communicate data and information." [@brazma2006standards]
...
can be developed that is *interoperable* with data from many research group's 
...
**Ontology standards.** Although it has the most complex name, an *ontology*
(sometimes called a *terminology* [@sansone2012toward]) might be the easiest and
...
between ideas or concrete things in a research area [@ghosh2011software], but 
...
biomedical research [@ghosh2011software]. Existing community-level ontologies in
...
(http://bioportal.bioontology.org/) provides access to almost 800 ontologies,
...
datapoints or written documents or computer programs [@kernighan1984unix]. As
...
\noindent A *file format* defines the rules for how the bytes in the chunk of
...
memory for that file. Because most ".pdf" files conform to the same *file
...
the the *FCS file format*. This format has been defined through a series of
...
Initiative) [@ghosh2011software], while in some cases the file format developed
...
For an even simpler example, thing about recording dates. The *minimum
...
collected and recorded in *.fcs* files. Every piece of flow cytometry equipment
can then be built to output data in this format, and every piece of software to
analyze flow cytometry data can be built to read in this input. The *.fcs* file
...
For this data recording, the data may be written down in an *ad hoc*
...
This does require some extra time commitment [@brazma2006standards]. First, time
...
### Saving two-dimensional structured data in plain text file formats

If you have recorded data in a two-dimensional structured format, you can choose
to save it in either a *plain text* format or a *binary* format. With a plain
text format, a file is "human readable" when it's opened in a text editor
[@hunt2000pragmatic; @janssens2014data], because each byte that encodes the file
translates to a single character [@murrell2009introduction], usually using an
...
[@buffalo2015bioinformatics], other more complex file formats like SAM and XML
...
different binary file format [@murrell2009introduction]. Some binary file
...
typically outweigh these limitations [@hunt2000pragmatic]. Advantages include:
(1) humans can read the file directly [@hunt2000pragmatic; @janssens2014data],
...
require specialized software [@murrell2009introduction]; (3) the
...
inputting and outputtin line-based plain-text files [@janssens2014data]; and (4)
plain-text files can be easily tracked with version control
[@hunt2000pragmatic]. These advantages might become particularly important in
...
The book *The Pragmatic Programmer* highlights some of the advantages of plain
text:
...
that standard." [@hunt2000pragmatic]
...
Adobe can understand this thing." [@ford2015on]
...
[@janssens2014data]. While the open proprietary format is preferable, since
...
that data in a plain text file format [@murrell2009introduction].  In most
spreadsheet programs, you can choose to save a file "As CSV".

### Occassions for more complex data structures and file formats
...
data stored in different formats [@murrell2009introduction].

**Non-tabular plain-text formats.**
...
better to use a non-tabular data format [@raymond2003art]. Popular data formats
...
return data in these formats [@janssens2014data].
...
[@buffalo2015bioinformatics]. This *streaming* approach is sometimes used with
...
considering using a database to store the data [@murrell2009introduction].
...
(*relations*) between different pieces of data and allows you to access
different combinations of that data quickly using Structured Query Language, or
*SQL* [@ford2015i]. Further, some statistical programming languages, including
...
pocket-sized database, built in." [@ford2015i]
...
including SQLite [@buffalo2015bioinformatics].
...
[@kernighan1984unix; @hunt2000pragmatic]. For very large biomedical datasets,
binary file formats are sometimes designed for *out-of-memory approaches*
[@buffalo2015bioinformatics], where a file format is designed in a way that
allows computer programs to find and read only specific pieces of data in a file
through a process called *random access*, rather than needing to read the full
file into memory before a specific piece of data in the file can be accessed
(a.k.a., *sequential access*) [@murrell2009introduction].
...
equipment from different manufacturers [@schadt2010computational].
