...
[@beckman2021implementing]. This would be practically unimaginable without the
...
[@raymondunderstanding]. Although they can work with projects that include
...
step is called *merging*. While this is a feature driven by the Git software
...
about the same time. These are called *merge conflicts*. They're places in the
...
tools for project management. These include *issue trackers*, which allow the
team to keep a running "to-do" list of what needs to be done to complete the
project [@perez2016ten]. Sometimes the best tools also happen to be those that
...
the course of the project [@metz2015github]. A private project can be viewed
...
research questions [@perez2016ten].
...
@raymond2003art); new tools are making the process even easier. The GitHub Pages
functionality, for example, is now being used to host a number of books created
in R using the `bookdown` package, including the online version of this book
[@bookdown]. The `blogdown` package similarly can be used to create websites,
either for individual researchers, for research labs, or for specific projects
or collaborations [@blogdown]. Further, if a project includes the creation of
...
enabling a community of coders [@perez2016ten]. Because of this
