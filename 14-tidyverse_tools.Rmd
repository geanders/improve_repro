...
types of calculations from scratch. 

In the tidyverse, you can add or change columns in the dataframe to store these
types of scaled values using the `mutate` function from the `dplyr` package, and
in general, the `dplyr` package is a key package to learn from the tidyverse, as
...
of conversion can easily be done using functions from the `dplyr` package. 
...
There are also more complex algorithms that can be used for standardizing
...
steps of data processing have been completed.
...
use useful information in them.
...
Another handy set of tools is a set for working with dates and times. Often, you
...
continue working with the output. 

A broader set of tools has been developed more recently. The `tidytools` 
package [?] aims to create a common interface ...
